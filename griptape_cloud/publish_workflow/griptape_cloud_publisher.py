...
class GriptapeCloudPublisher:
    ...
    def publish_workflow(self) -> ResultPayload:
        try:
            ...
            structure_id = self._deploy_workflow_to_cloud(package_path)
            logger.info("Workflow '%s' published successfully to Structure: %s", self._workflow_name, structure_id)

            # self._update_workflow_metadata_with_published_id(
            #     workflow_name=self._workflow_name,
            #     published_workflow_id=response["id"],
            # )
            # logger.info("Workflow '%s' metadata updated with published ID: %s", self._workflow_name, id)

            return PublishWorkflowResultSuccess(
                workflow_id=structure_id,
            )
    ...
    def _deploy_workflow_to_cloud(self, package_path: str) -> str:
        ...
        return create_structure_response.structure_id
